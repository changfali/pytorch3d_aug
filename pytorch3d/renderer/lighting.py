...
def diffuse_v06(normals, color, direction) -> torch.Tensor:
    """
    Calculate the diffuse component of light reflection using Lambert's
    cosine law.
...
def specular_v06(points, normals, direction, color, camera_position, shininess, highlight="phong") -> torch.Tensor:
    """
    Calculate the specular component of light reflection.
    ...
    normals = F.normalize(normals, p=2, dim=-1, eps=1e-6)
    direction = F.normalize(direction, p=2, dim=-1, eps=1e-6)
    specularity = specular_highlight(normals, direction, camera_position, points, shininess)
...
class PointLights_v06(TensorProperties):
    def __init__(
        self,
        ambient_color=((0.5, 0.5, 0.5),),
        diffuse_color=((0.3, 0.3, 0.3),),
    ...
    def diffuse(self, normals, points) -> torch.Tensor:
        ...
        if len(self.diffuse_color.shape) > 2:
            ...
            for light_no in range(self.diffuse_color.shape[1]):
                ...
                diff_comp += diffuse_v06(normals=normals, color=self.diffuse_color[:, light_no, :], direction=direction)

        else:
            direction = location - points
            diff_comp = diffuse_v06(normals=normals, color=self.diffuse_color, direction=direction)
