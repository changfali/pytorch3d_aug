...
from pytorch3d.io.utils import _check_faces_indices, _make_tensor, _open_file, PathOrStr, _read_image_simple
from pytorch3d.renderer import TexturesAtlas, TexturesUV, TexturesUV_v06
...
def load_objs_and_textures(
    files: list,
    diffAlbedos: list = None,
    specAlbedos: list = None,
    specNormals: list = None,
    diffNormals: list = None,
    shininess: list = None,
    vertices_uvs: list = None,
    shadows: list = None,
    translucency: list = None,
    device=None,
):
    """
    Load meshes from a list of .obj files using the load_obj function, and
    ...
    for i, f_obj in enumerate(files):
        ...
        verts, faces, aux = load_obj_v06(f_obj, load_textures=True, tex_path=tex_path)
        ...
        if tex_maps is not None and len(tex_maps) > 0:
            ...
            tex = TexturesUV_v06(verts_uvs=verts_uvs, faces_uvs=faces_uvs, maps=image, **reflectance)
...
def _load_obj(
    ...
    *,
    ...
    if len(faces_normals_idx):
        faces_normals_idx = _format_faces_indices(faces_normals_idx, normals.shape[0], device=device, pad_value=-1)
    if len(faces_textures_idx):
        faces_textures_idx = _format_faces_indices(faces_textures_idx, verts_uvs.shape[0], device=device, pad_value=-1)
...
def _write_normals(normals: torch.Tensor, faces_normals_idx: torch.Tensor, float_str: str) -> str:
    ...
    for i in range(F):
        if faces_normals_idx is not None:
            if faces_uvs is not None:
                ...
                face = ["%d//%d" % (faces[i, j] + 1, faces_normals_idx[i, j] + 1) for j in range(P)]
